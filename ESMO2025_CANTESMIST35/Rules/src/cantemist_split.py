...
cantemist_split.py
------------------
Deterministic 75/25 train/test split of CANTEMIST35 documents
...
Run once:  python cantemist_split.py
"""
...
def build_split(*, force=False):
    ...
    train, test = _iterative_stratification(ids, doc_labels, TEST_FRACTION, rng)
...
def _print_stratification_report(doc_labels, train, test):
    ...
    all_labels = sorted({l for ls in doc_labels.values() for l in ls})
    print(f"\n{'label':<65}{'train%':>8}{'test%':>8}{'train_n':>9}{'test_n':>8}")
    for l in all_labels:
        ...
        if total == 0:
            continue
        print(f"{l:<65}{n_train/total*100:>7.1f}%{n_test/total*100:>7.1f}%"
