"""
PubMedBERT fine-tuning + evaluation for NER on MACCROBAT2020.
...
Pipeline:
  ...
  3. Fine-tune PubMedBERT with HuggingFace Trainer
  4. Evaluate on test set (token-level seqeval + span-level for GLiNER comparison)
  5. Save results to pubmedbert_results.csv + training plots

Usage:
    pip install transformers datasets seqeval accelerate matplotlib
    python drbert_ner_eval.py                  # fine-tune + eval
    python drbert_ner_eval.py eval             # eval only (load saved model)
    python drbert_ner_eval.py inspect 0        # inspect doc index 0 from test set
"""
...
OUTPUT_DIR = DATA_DIR / "pubmedbert_finetuned"
RESULTS_CSV = DATA_DIR / "pubmedbert_results.csv"
PARAMS_CSV = DATA_DIR / "pubmedbert_train_params.csv"
...
# warmup_steps = ~1 epoch = 2381 / (8*2) = 149
TRAIN_PARAMS = dict(
    num_train_epochs=25,
    learning_rate=3e-4,
...
def plot_training(log_history, train_f1, test_f1):
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.set_title(f"Loss — {MODEL_SHORT} (MACCROBAT2020)")
    ax.legend()
    ...
    ax.set_ylim(0, 1)
    ax.set_ylabel("F1")
    ax.set_title(f"Overfitting check — {MODEL_SHORT} (MACCROBAT2020)")
    ...
    plt.tight_layout()
    existing = sorted(PLOTS_DIR.glob(f"{MODEL_SHORT}__training_run*.png"))
    run_num = len(existing) + 1
    out_path = PLOTS_DIR / f"{MODEL_SHORT}__training_run{run_num}.png"
...
def fine_tune():
    ...
    print(f"BIO labels ({len(BIO_LABELS)}): {BIO_LABELS[:5]} ... ({len(BIO_LABELS)} total)")
...
def evaluate_model(model, tokenizer, train_data, test_data, log_history=None):
    ...
    for label in MACCROBAT_LABELS:
        ...
        print(f"    {label:<30}  F1={lf1:.3f}  (TP={ltp} FP={lfp} FN={lfn})")
