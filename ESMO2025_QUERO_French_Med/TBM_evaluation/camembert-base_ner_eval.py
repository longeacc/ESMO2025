"""
CamemBERT-base + LoRA fine-tuning + evaluation for NER on QUAERO FrenchMed.
...
OUTPUT_DIR = DATA_DIR / "drbert_finetuned"
RESULTS_CSV = DATA_DIR / "drbert_results.csv"
PARAMS_CSV = DATA_DIR / "drbert_train_params.csv"
...
def plot_training(log_history, train_f1, test_f1):
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.set_title("Loss — CamemBERT-base")
    ax.legend()
    ...
    ax.set_ylim(0, 1)
    ax.set_ylabel("F1")
    ax.set_title("Overfitting check — CamemBERT-base")
    ...
    existing = sorted(PLOTS_DIR.glob("CamemBERT-base__training_run*.png"))
    run_num = len(existing) + 1
    out_path = PLOTS_DIR / f"CamemBERT-base__training_run{run_num}.png"
...
def evaluate_model(model, tokenizer, train_data, test_data, log_history=None):
    ...
    for label in QUAERO_LABELS:
        ...
        print(f"    {label:<6}  F1={lf1:.3f}  (TP={ltp} FP={lfp} FN={lfn})")
